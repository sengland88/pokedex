...
$("#pause-play-music").on("click", function(event) {
    if (sound.paused) {
        return sound.play();
    }   else if (sound.play) {
        return sound.pause()
...
//Button that allows the user to go back to the opening message page.
$("#the-message").on("click", function(event) {
    $(".theMessage").show()
    $(".startUp").hide()
    $(".second-button-group").hide()
    $(".theDex").hide()
...
//Toggle button that shows and hides the pokedex when clicked.
$("#hide-dex").on("click", function(event) {
    $(".theDex").toggle();
})
